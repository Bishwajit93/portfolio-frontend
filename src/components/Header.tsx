    ...
    >
      <div className="flex items-center h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 border-b border-blue-500/20 shadow-[0_4px_20px_rgba(0,191,255,0.4)]">
        ...
        <div className="flex-1 min-w-0">
          <Link
            href="/"
            className="text-sm md:text-lg font-extrabold text-cyan-300 tracking-wide hover:text-white transition block truncate"
          >
            Bishwajit Karmaker
          </Link>
        </div>
        ...
        <div className="hidden md:flex flex-1 justify-center">
          <ul className="flex items-center space-x-5 text-sm font-semibold text-cyan-300">
            {navItems.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
        ...
        {/* RIGHT: Logout (stays on the right, never gets pushed away) */}
